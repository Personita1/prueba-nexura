...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        <div class="container pr-5 pl-5 mt-4">
            ...
            @endif
            <form id="formEditarEmpleado" action="/actualizar/{{ $empleado->id }}" method="post">
                @csrf
                <div class="mb-3 row">
                    <label for="inputNombre" class="col-sm-3 col-form-label fw-bold text-end" required>Nombre completo * </label>
                    <div class="col-sm-9">
                        <input type="text" class="form-control" id="inputNombre" name="nombre" value="{{ $empleado->nombre }}">
                    </div>
                </div>
                <div class="mb-3 row">
                    <label for="inputEmail" class="col-sm-3 col-form-label fw-bold text-end"  >Correo electrónico * </label>
                    <div class="col-sm-9">
                        <input type="email" class="form-control" id="inputEmail" name="email" value="{{ $empleado->email }}">
                    </div>
                </div>
                <div class="mb-3 row">
